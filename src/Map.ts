...
class TilemanMap {
  ...
  public async rangeBuy(area : MapRange) : Promise<void> {
    const playableArea : MapRange = Map.getPlayableArea();
  ...
  private _getValidTiles(area : MapRange, action : LandOwnershipAction) : ValidTilesResult {
    ...
    const clampedArea : MapRange = clampRange(area, Map.getPlayableArea());
...
export const Map : TilemanMap = new TilemanMap();